...
namespace Microsoft.WimgApi
{
    ...
    WindowsImageContainer : IDisposable
    {
        ...
        //[CLSCompliant(false)]
        public WindowsImageContainer(string imageFilePath, CreateFileMode mode, CreateFileAccess access,bool Create_Image)
        {
            ...
            if (!m_ImageContainerHandle.Equals(IntPtr.Zero))
            {
                ...
                /*if (Create_Image)
                {
                    NativeMethods.SetTemporaryPath(m_ImageContainerHandle, @"\\1.1.1.2\ext\Temp\");
                }
                else
                {
...
(String.Equals(childNode.Name, node.ToString(), StringComparison.InvariantCultureIgnoreCase))
        ...
        NativeMethods
        {
            ...
            CreateFile(string imageFile, uint access, uint mode)
            {
                ...
                windowsImageHandle = NativeMethods.WimCreateFile(imageFile, access, mode, 0, 2, out creationResult);
